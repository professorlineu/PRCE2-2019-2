...
int main()
{
    ...
    cout << "\nConsidere: 'V = Compra a vista' e 'P = Compra a prazo'" << endl;

    while (iCont <= fTransacao)
    {
        iCont;

        cout << "\nA compra foi a prazo ou à vista?";
        cin >> cCod;
    ...
    if(fPagPra > 0)
    {
        cout << "O total das compras à prazo é: R$ " << fPagPra << endl;
        cout << "O valor da primeira prestação é: R$ " << fPagPra/iParcela << endl;
